...
/**
 * This class is used to create implementations of XML Pull Parser defined in XMPULL V1
 * API. The name of actual factory class will be determined based on several parameters.
 * It works similar to JAXP but tailored to work in J2ME environments (no access to system
 * properties or file system) so name of parser class factory to use and its class used
 * for loading (no class loader - on J2ME no access to context class loaders) must be
 * passed explicitly. If no name of parser factory was passed (or is null) it will try to
 * find name by searching in CLASSPATH for
 * META-INF/services/org.xmlpull.XmlPullParserFactory resource that should contain a comma
 * separated list of class names of factories or parsers to try (in order from left to the
 * right). If none found, it will throw an exception.
 *
 * <br />
 ...
 * <code>System.getProperty(XmlPullParserFactory.PROPERTY_NAME)</code> and second is
 * <code>Thread.getContextClassLoader().getClass()</code> .
 *
 * @see XmlPullParser
...
public class XmlPullParserFactory {
	...
	/**
	 * Name of the system or midlet property that should be used for a system property
	 * containing a comma separated list of factory or parser class names (value:
	 * org.xmlpull.XmlPullParserFactory).
	 */

	public static final String PROPERTY_NAME = "org.xmlpull.XmlPullParserFactory";
	 ...
	 * @param awareness true if the parser produced by this code will provide support for
	 * XML namespaces; false otherwise.
	 */
	 ...
	 * namespace aware (it simply set feature XmlPullParser.FEATURE_PROCESS_NAMESPACES to
	 * true or false).
	 * @return true if the factory is configured to produce parsers which are namespace
	...
	/**
	 * Specifies that the parser produced by this factory will be validating (it simply
	 * set feature XmlPullParser.FEATURE_VALIDATION to true or false).
	 *
	 ...
	 * @return true if the factory is configured to produce parsers which validate the XML
	...
	/**
	 * Creates a new instance of a XML Pull Parser using the currently configured factory
	 * features.
	...
	/**
	 * Create a new instance of a PullParserFactory that can be used to create XML pull
	 * parsers (see class description for more details).
	 * @return a new instance of a PullParserFactory, as returned by newInstance (null,
	 * null);
	 */
	...
	public static XmlPullParserFactory newInstance(String classNames, Class context) throws XmlPullParserException {
		...
		if (context == null) {
			...
			// this is the best we can do without having access to context classloader in
			// J2ME
